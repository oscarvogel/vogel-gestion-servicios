...
def test_equipment_cannot_cross_customer_tenant(client,db_session):
    ...
    ok=client.post(f"/api/v1/customers/{ca['id']}/equipment",headers=ha,json={"customer_id":ca["id"],"category":"TV","brand":"Samsung"})
    assert ok.status_code==201,ok.text
    denied=client.get(f"/api/v1/customers/{ca['id']}/equipment",headers=hb)
    assert denied.status_code==404
    mismatch=client.post(f"/api/v1/customers/{cb['id']}/equipment",headers=hb,json={"customer_id":ca["id"],"category":"Notebook"})
