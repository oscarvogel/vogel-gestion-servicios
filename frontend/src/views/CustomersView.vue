...
interface Equipment { id:number; customer_id:number; category:string; brand:string|null; model:string|null; serial_number:string|null; description:string|null; notes:string|null; active:boolean }
...
const eq=ref({category:"",brand:"",model:"",serial_number:"",description:"",notes:""});
...
function newEquipment(){eq.value={category:"",brand:"",model:"",serial_number:"",description:"",notes:""};showEquipment.value=true}
async function saveEquipment(){if(!selected.value)return;try{await apiPost("/customers/"+selected.value.id+"/equipment",{customer_id:selected.value.id,...eq.value});showEquipment.value=false;await openCustomer(selected.value);toast.push("Equipo agregado","success")}catch(e){toast.push(getApiErrorMessage(e),"error")}}
...
<template>
  ...
  <div v-if="selected" class="card"><div class="flex flex--between"><div><p class="card__title">Equipos de {{ selected.name }}</p><p class="text-secondary">Cada equipo pertenece sólo a esta empresa.</p></div><button class="btn btn--primary" @click="newEquipment">+ Agregar equipo</button></div>
    <table v-if="equipment.length" class="table"><thead><tr><th>Tipo</th><th>Marca / modelo</th><th>Serie</th><th>Descripción</th></tr></thead><tbody><tr v-for="e in equipment" :key="e.id"><td>{{e.category}}</td><td>{{[e.brand,e.model].filter(Boolean).join(' ')||'—'}}</td><td>{{e.serial_number||'—'}}</td><td>{{e.description||'—'}}</td></tr></tbody></table>
  ...
  <Modal :open="showEquipment" :title="'Nuevo equipo · '+(selected?.name||'')" @close="showEquipment=false"><form @submit.prevent="saveEquipment">
    <div class="field__row"><div class="field"><label>Tipo / categoría *</label><input v-model="eq.category" placeholder="TV, notebook, motor…" required></div><div class="field"><label>Marca</label><input v-model="eq.brand"></div></div>
